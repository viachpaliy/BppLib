...
namespace BppLib.CixParser
{
	...
	{
		/// <summary> Parses lines of code and returns the <c>Oval</c> instance.</summary>
		...
		public static Oval ParseOval(string[] code)
		{
			...
			foreach(var s in code)
			{
				...
				if (m.Success)
				{
					...
					if (name == "R1") obj.R1 = ConvertToDouble(value);
					if (name == "X2") obj.X2 = ConvertToDouble(value);
					if (name == "Y2") obj.Y2 = ConvertToDouble(value);
					if (name == "R2") obj.R2 = ConvertToDouble(value);
					if (name == "LKR") obj.Lkr = ConvertToDouble(value);
