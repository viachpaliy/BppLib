...
namespace BppLib.CixParser
{
	...
	{
		/// <summary> Parses lines of code and returns the <c>Star</c> instance.</summary>
		...
		public static Star ParseStar(string[] code)
		{
			...
			foreach(var s in code)
			{
				...
				if (m.Success)
				{
					...
					if (name == "ER") obj.Er = ConvertToDouble(value);
					if (name == "IR") obj.Ir = ConvertToDouble(value);
					if (name == "PS") obj.Ps = ConvertToInt(value);
